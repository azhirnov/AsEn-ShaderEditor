...
* Увеличивается нагрузка на вершинный шейдер. Для однопиксельных треугольников значение приближается к одному VS на пиксель, что уже много. Для треугольников с уникальными вершинами, например листьев дерева, уже 3 вызова VS на пиксель.
...
* Для оптимизации доступа к памяти нужно хранить позицию вершин отдельно от остальных аттрибутов.
...
* [Siggraph2015: GPU-Driven Rendering Pipelines (слайды 29-36)](https://advances.realtimerendering.com/s2015/aaltonenhaar_siggraph2015_combined_final_footer_220dpi.pdf)
...
* [Siggraph2015: GPU-Driven Rendering Pipelines](https://advances.realtimerendering.com/s2015/aaltonenhaar_siggraph2015_combined_final_footer_220dpi.pdf) - начиная со слайда 39 рассказывают про Virtual Deferred Texturing. Получился 64-битный GBuffer + depth (24 или 32 бита).
...
* Особенно эффективен для геометрии с альфа-тестом, где треугольники хуже отсекаются по EarlyZ.
* Требует разбиение геометрии на кластеры, чтобы уплотнить индексы треугольников и материалов, иначе потребуется больше места в G-буфере.
...
* Чтобы полностью избавиться от quad overdraw нужна софтварная растеризация как в Nanite, HFW.
...
* На TBDR архитектуре создание VisBuf в 2 раза медленее чем depth pre-pass, так как для вершин сохраняется PrimitiveID и материал, что увеличивает объем памяти бинов, которые выгружаются в глобальную память
* Трансформация трех вершин, интерполяция аттрибутов и расчет дериватив на каждый фрагмент очень затратно для слабых ГП.
...
* Не сказано как решать проблему выбора мипов без дериватив.
...
В более старых вариантах отсечение идет на стороне ЦП по заранее уменьшенному буферу глубины.
Из минусов отставание в несколько кадров чтобы не ждать завершения рисования, это снижает эффективность.
...
1. Сначала уменьшить до степени 2, затем расчитывать мип-уровни. [Пример](https://github.com/azhirnov/AsEn-ShaderEditor/blob/main/src/scripts/geom-cull/perf-GenHiZ-1.as).
...
2. Для каждого мип уровня выбирать какие пиксели из верхнего уровня влияют на него. [Пример](https://github.com/azhirnov/AsEn-ShaderEditor/blob/main/src/scripts/geom-cull/perf-GenHiZ-2.as).
	- Точность теряется только когда 3 пикселя переходят в 1.
	- Такой вариант хуже по производительности.
3. Использовать смешанный подход - округлить разрешение до небольшой степени 2, например 32, и использовать первый вариант для этих мипов, затем второй вариант для оставшихся.
	- Дает лучшую производительность на высоком разрешении.
	- Дает лучшую точность на меньших мипах.
	- Для нижних мипов можно использовать генерацию за один проход в компьют шейдере.
...
Идея в том, чтобы сделать растеризацию простой геометрии типа AABB и OOB, либо 2-3 пересекающихся плоскостей, что еще проще.

* Хорошо подходит, когда ЦП слабо нагружен.
* Подходит для времен x64, а сейчас из-за популярности ARM, появления RISC-V, AVX512 на AMD Zen4, портирование и оптимизация займет много времени, в отличие от кроссплатформенных шейдеров.
* В целом компиляторы C++ хуже оптимизируют SIMD, видеокарты с этим справляются намного лучше.
* Менее точный чем HiZ с мешлетами, так как использует большую геометрию.
* Тот же принцип может использоваться для софтварной реализации на стороне ГП.
* В итоге с переходом на GPU driven pipeline от такого подхода начали отказываться еще в 2015.

Ссылки:
* [Intel Software Occlusion Culling](https://github.com/rygorous/intel_occlusion_cull)
...
* [Coverage Bitmasks](https://media.contentapi.ea.com/content/dam/ea/seed/presentations/seed-coverage-bitmasks-mittring.pdf) - вариант растеризации на ГП.
...
* Coarse rasterization этап тестирует пересечение треугольника с тайлом 8х8 (AMD GCN).
* На этом этапе применяется встроенный иерархический буфер глубины (HiZ), который отсекает целые тайлы без необходимости тестировать каждый квадрат или пиксель.
* HiZ тайла хранит максимальное и минимальное значение глубины для всего тайла и для более мелких тайлов внутри.
* Если треугольник частично перекрыт, то HiZ позволяет отбрасывать части 4х4 или 2х2 до попиксельного теста.
* При записи в глубину также обновляется и HiZ.
* При очистке глубины срабатывает быстрая очистка, когда меняется только HiZ значение всего тайла.
...
* Квадраты, которые прошли тест, далее группируются, чтобы занять все свободные потоки варпа. На этом основаны старые способы классификации без компьют шейдера.
* Оптимизация выключается, если фрагментный шейдер меняет глубину через `gl_FragDepth`.

Discard в шейдере и прозрачность:
* Не меняют глубину, поэтому также могут отсекаться по EarlyZ.
* На старых ГП нет механизма обновления/синхронизации LateZ записи с EarlyZ тестом следующих полигонов, поэтому EarlyZ отключается и производительность снижается.
...
* Для полигонов с discard в шейдере отсечение на этапе нарезки не работает.
...
Получается, что техники понижения разрешения при временном снижении производительности хуже работают без понижения детализации геометрии.
...
А AMD RX570 без сортировки замедляется в 2-3 раза, Intel N150 медленее на 70%.
...
Mali FPK не работает на мелких треугольниках, поэтому в тестах он показывает нестабильный результат.

Очень хорошо показал себя Adreno LRZ, потери без сортировки не более 10%.

У PowerVR схожее поведение с Mali FPK, только в 4К потери уменьшаются с 2х до 20%.

У Apple похоже используется аналог FPK с такими же худшими случаями.
...
VisBuf и DeferTex как и ожидалось уменьшает quad overdraw, что особенно важно при нагрузке на ALU.
...
На остальных (NV RTX2080, AMD 780M, Intel UHD620, Intel N150) DPP всего на 20-30% быстрее.

В целом VisBuf и DeferTex намного эффективнее DPP за счет одного прохода с растеризацией при легком FS, либо за счет отсутствия quad overdraw при тяжелом FS.
...
Трансформация трех вершин на пиксель очень затратна для слабых мобилок, поэтому на медленном железе намного лучше работает DeferTex метод, хоть и не дает такой же гибкости как VisBuf.
...
Можно расчитать нагрузку на память как количество пикселей плюс примерно 20% на пересекающиеся AABB. Для 4К получаем 40МБ/к или 2.4ГБ/с при 60фпс.
По тестам Raster occlusion в 2К/4К работает в 2-4 раза медленее HiZ при одинаковой по рассчетам нагрузке на память.
...
На старых ГП отключается EarlyZS оптимизация, в результате производительность снижается до уровня, когда буфер глубины не используется вовсе. Но это все еще быстрее, чем изменение глубины в фрагментном шейдере.

На Adreno 505, Mali G57, Intel UHD 620 отключается EarlyZS оптимизация.

На более свежих ГП запись в глубину учитывается в EarlyZS, в результате полностью перекрытые полигоны не вызывают фрагментный шейдер.

Производительность снижается у Adreno 660 (50-70%), Mali G610 (в 4К на 30%), PowerVR (20-30%).
Нет изменений в производительности у Intel N150, Intel Arc140T, AMD 780M, AMD RX570, NV RTX, Mali G610 (в 2К), Apple.
