...
* Для равномерного распределения точек на сфере.
...
**3D Вороной**
Из плюсов - сразу дает круги без искажений, но на этом все.<br/>
...
Кратеры заданы 3D сферой на поверхности планеты, расстояние до центра сферы используется для наложения кратера. Форма кратера задается комбинацией кривых и сглаживанием как в SDF ([подробнее в видео](https://youtu.be/lctXaT9pxA0?t=338)).<br/>
Главный недостаток - полный перебор всех кратеров для каждой вершины, придется использовать quad tree и выделять дополнительную память под это.
...
3. Центр ячейки проецируется в 3D и рассчитывается расстояние до нормали на поверхности сферы.
...
![](../../samples/res_editor/docs/img/CraterMerge.jpg)
...
### Каньёны, трещины и прочие линии

Сфера пересекается с плоскостью, для каждой точки считается расстояние до места пересечения. Далее считаются угловые координаты точки и получаются `UV` координаты для наложения текстуры.

К расстоянию до места пересечения сферы и плоскости можно добавить 3D шум, чтобы получить кривую, но тогда рассчитанный `UV` получится искаженным. Поэтому все искажения правильнее применять после расчета `UV`.
