...
Также есть разделение на скалярные и векторные регистры. Скалярные это 32бита на варп, а векторные - 32бита на поток варпа, то есть 32х32 для simd32.
...
Начиная с Valhall gen3 тайл увеличили до 32х32. Размер варпа зафиксирован на 16 потоков, скорее всего там simd8 с dual issue.
...
Максимальное расстояние между пикселями варпа оказалось 142. В таких случаях кластерный forward+ кажется не лучшим вариантом.
...
В Adreno сделали большой варп на 64 потока и 128.
...
Но это уже не применимо для параллакса с трассировкой по глубине, поэтому в современных ГП нарастили количество регистров и сделали быстрое переключение между варпами.
...
**SIMD/SIMT** - физический блок, который выполняет одну инструкцию над векторным регистром. Внутри содержит несколько пайплайнов: FMA, SFU, очередь запросов к памяти и тд.
...
**Варп** (warp, wave на AMD) - логический элемент, содержит: векторные и скалярные регистры, указатель на инструкцию, дополнительные параметры типа запросов к памяти и состояния ожидания данных.
...
Бонусом к этому идет возможность одну из половин в случае вырождения потоков, например из-за ветвления.
