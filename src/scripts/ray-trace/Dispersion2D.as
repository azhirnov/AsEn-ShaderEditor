...
/*
	Spectral ray tracing with dispersion in prism.
*/
...
//-----------------------------------------------------------------------------
	...
	void ASmain ()
	{
		...
		// create geometry for ray tracing
		{
		  ...
		  #if 1
			array<float2>	contour		= { float2(0.f, -0.25f), float2(0.25f, 0.25f), float2(-0.25f, 0.25f) };								// triangle
		  #else
		...
		// render loop
		{
			...
			pass.Slider(	"iLightDir",		0.f,			360.f,				167.f );
			pass.Slider(	"iLightShape",		0,				2,					1 );
			pass.Slider(	"iRayDepth",		1,				max_ray_depth-1,	max_ray_depth/2 );
			pass.Slider(	"iRecursionDepth",	1,				max_recursion,		max_recursion/3 );
			...
			draw.AddPipeline( "samples/Dispersion2D-area.as" );	// [src](https://github.com/azhirnov/AsEn-ShaderEditor/blob/main/src/pipelines/samples/Dispersion2D-area.as)
			...
			draw.AddPipeline( "samples/Dispersion2D-rays.as" );	// [src](https://github.com/azhirnov/AsEn-ShaderEditor/blob/main/src/pipelines/samples/Dispersion2D-rays.as)
...
//-----------------------------------------------------------------------------
	...
	bool  RayClosestHit (const float2 origin, const float2 dir, out HitParams hit)
	{
		...
		if ( GetCommittedIntersectionType( ray_query ) != gl::RayQueryCommittedIntersection::None )
		{
			...
			hit.mtrId		= mtr_id;
			hit.frontFace	= GetCommittedIntersectionFrontFace( ray_query );
	...
	bool  ConeClosestHit ()
	{
			 ...
			 (r0_params.mtrId == r1_params.mtrId)			)
		{
			...
			float			eta		= c_VacuumIOR / MaterialIOR( r0_params.mtrId, cone.wavelength );	// incident / transmitted
