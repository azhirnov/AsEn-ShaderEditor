...
/*
	Use 'Mode' to switch depth offset method.
*/
...
//-----------------------------------------------------------------------------
	...
	void ASmain ()
	{
		...
		RC<Scene>			scene_db		= Scene();		// with depth bias render state
		RC<FPVCamera>		camera			= FPVCamera();
		RC<DynamicUInt>		mode			= DynamicUInt();
		RC<DynamicFloat>	norm_bias		= DynamicFloat();
		...
		// add ground
		{
...

			{
				...
				UnifiedGeometry_DrawIndexed	cmd;
				cmd.PipelineHint( "opaque.DepthBias" );
				cmd.indexCount	= indices.size();
				cmd.IndexBuffer( geom_data, "indices" );
				geometry.Draw( cmd );

				scene_db.Add( geometry );
			}
		}
		...
		Slider( mode,		"Mode",			0,		2 );			// 0 - hw depth bias
																	// 1 - software depth bias in FS
																	// 2 - only world pos offset by normal
		Slider( sm_pot,		"ShadowDim",	0,		3,		2 );
		Slider( norm_bias,	"NormBias",		0.0,	2.0,	0.0 );	// 0.5 - best result
		...
		// render loop
		{
			...
			RC<SceneGraphicsPass>	pass = scene_db.AddGraphicsPass( "shadow depth bias" );
			pass.AddPipeline( "samples/LowPolyTrees/SM.as" );	// [src](https://github.com/azhirnov/AsEn-ShaderEditor/blob/main/src/pipelines/samples/LowPolyTrees/SM.as)
			...
			RC<SceneGraphicsPass>	pass = scene.AddGraphicsPass( "shadows" );
			pass.AddPipeline( "samples/LowPolyTrees/SM2.as" );	// [src](https://github.com/azhirnov/AsEn-ShaderEditor/blob/main/src/pipelines/samples/LowPolyTrees/SM2.as)
			pass.Output( "out_Color",		sm_col,		RGBA32f(0.0) );
			pass.Output(					sm,			DepthStencil(1.0, 0) );
			pass.ArgIn(  "un_Params",		cbuf );
			pass.Slider( "iConstBias",		0.0,	1.0,	0.5 );
			pass.Slider( "iSlopeBias",		0.0,	1.0,	0.5 );
			pass.Slider( "iBiasClamp",		0.0,	1.0,	0.0 );
			...
			pass.Constant( "iNormBiasScale", norm_bias );
		}
